...
class ParseModel(BaseModel):
    ...
    def load_pretrain_models(self,):
        ...
        self.netP.module.load_state_dict(torch.load(self.opt.parse_net_weight), strict=True)
    ...
    #     self.pred_Parse, self.img_SR = self.netP(input)
    #     if self.opt.debug:
    #         print('ParseNet output shape', self.pred_Parse.shape, self.img_SR.shape)
    ...
    def get_current_visuals(self, size=224):
        ...
        visual_imgs.append(np.array(utils.color_parse_map(self.pred_Parse)))
        visual_imgs.append(utils.color_parse_map(self.gt_Parse, size))
        visual_imgs.append(out_imgs[2])

        return visual_imgs
        
    def get_demo_img(self, input):
        pred_Parse, img_SR = self.netP(input)

        return pred_Parse, img_SR