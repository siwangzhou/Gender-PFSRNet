...
class PSFRGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, in_size=224, out_size=224, min_feat_size=14, ngf=64, n_blocks=9, parse_ch=19, relu_type='relu',
            ch_range=[32, 1024], norm_type='spade'):
        super().__init__()
        
        min_ch, max_ch = ch_range
        ch_clip = lambda x: max(min_ch, min(x, max_ch))
        get_ch = lambda size: ch_clip(1024*min_feat_size//size)
