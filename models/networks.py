...
def define_P(opt, in_size=224, out_size=224, min_feat_size=14, relu_type='LeakyReLU', isTrain=True, weight_path=None):
    net = ParseNet(in_size, out_size, min_feat_size, 64, 19, norm_type=opt.Pnorm, relu_type=relu_type, ch_range=[32, 512])
    if not isTrain:
        net.eval()  
...
class UNetGeneratorSN(nn.Module):
    ...
    def __init__(self, num_in_ch, num_out_ch, num_feat=64, skip_connection=True):
        super(UNetGeneratorSN, self).__init__()
...
class ResidualBlockForGen(nn.Module):
    """Residual Block with instance normalization."""
    def __init__(self, dim_in, dim_out):
        super(ResidualBlockForGen, self).__init__()
...
class Generator(nn.Module):
    ...
    def __init__(self, conv_dim=64, c_dim=5, repeat_num=6):
        ...
        for i in range(repeat_num):
            layers.append(ResidualBlockForGen(dim_in=curr_dim, dim_out=curr_dim))
...
if __name__ == '__main__':
    ...
    model = UNetGeneratorSN(num_in_ch=3, num_out_ch=3)
